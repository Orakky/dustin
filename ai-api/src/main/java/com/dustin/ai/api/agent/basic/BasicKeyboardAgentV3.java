...
@Component(name = "basicKeyboardAgentV3")
public class BasicKeyboardAgentV3 {
    ...
    /**
     * process multi intent actions and return flux assistant message - v3 version
     * @param actionableIntents
     * @param message
     * @param sessionId
     * @param username
     ...
     * @return
     */
    private Flux<AssistantMessage> handleMultiIntentActionsV3(List<IntentAnalysis> actionableIntents,
                                                            String message, String sessionId,
                                                            String username, ChatSessionTool session) throws IOException {
        //sort by confidence
        actionableIntents.sort(Comparator.comparingInt(IntentAnalysis::getConfidence).reversed());

        //create tools execute plan
        ToolExecutionPlan executionPlan = createExecutionPlan(actionableIntents,message,sessionId,username,session);

        // execute tool plan
        ToolExecutionContext context = executeToolPlan(executionPlan,sessionId,username,session);

        return generateFinalResponse(context,session);
    }

    /**
     * generate final reponse
     * @param context
     * @param session
     * @return
     */
    private Flux<AssistantMessage> generateFinalResponse(ToolExecutionContext context, ChatSessionTool session) throws IOException {
        StringBuilder combinedResults = new StringBuilder();
        combinedResults.append("工具执行汇总:\n\n");

        for (ToolExecutionResult result : context.getTaskResults().values()) {
            ToolTask task = result.getTask();
            combinedResults.append("任务").append(task.getTaskIndex())
                    .append(" (").append(task.getServiceName()).append("): ")
                    .append(task.isSuccess() ? "成功" : "失败")
                    .append("\n");

            if (task.isSuccess()) {
                combinedResults.append("结果: ").append(result.getResult()).append("\n\n");
            } else {
                combinedResults.append("错误: ").append(task.getErrorMessage()).append("\n\n");
            }
        }

        SystemMessage summaryMessage = ChatMessage.ofSystemTmpl(
                        "工具执行完成，请综合分析以下结果并回答用户原始问题：\n${combinedResults}")
                .paramAdd("combinedResults", combinedResults.toString())
                .generate();
     ...
     * @return
     */
    private ToolExecutionContext executeToolPlan(ToolExecutionPlan executionPlan,
                                                 String sessionId, String username, ChatSessionTool session) {
        ...
        // sorted task
        List<ToolTask> executableTasks = getExecutableTasks(executionPlan.getTasks());

        //
        for (ToolTask task : executableTasks) {
            try{
                // 更新任务状态
                task.setStatus(ToolTaskStatus.RUNNING);
                task.setStartTime(System.currentTimeMillis());
                updateTaskStatus(task, context);

                // 解析参数（处理变量引用）
                Map<String, Object> resolvedParams = resolveTaskParameters(task, context);

                // 记录执行日志
                logToolExecution(session, task, resolvedParams);

                // 执行工具
                String result = executeSingleTool(task.getServiceName(), resolvedParams);

                // 更新任务结果
                task.setStatus(ToolTaskStatus.COMPLETED);
                task.setEndTime(System.currentTimeMillis());
                task.setResult(result);
                task.setSuccess(true);

                // 保存结果到上下文
                context.getTaskResults().put(task.getTaskId(), new ToolExecutionResult(task, result));
                context.getToolResults().put(task.getServiceName(), result);

                // 持久化执行结果
                persistExecutionResult(context, task);

            }catch (Exception e){
                log.error("tool task fail, taskId: {}", task.getTaskId(), e);
                task.setStatus(ToolTaskStatus.FAILED);
                task.setEndTime(System.currentTimeMillis());
     ...
     * 持久化执行结果（简化实现）
     */
    private void persistExecutionResult(ToolExecutionContext context, ToolTask task) {
        // 在实际项目中，这里应该保存到数据库或文件系统
        String resultJson = JSONUtil.toJsonStr(new ToolExecutionResult(task, task.getResult()));
        log.info("持久化执行结果 - 任务{}: {}", task.getTaskId(), resultJson);
    }

    /**
     * 执行单个工具
     */
    private String executeSingleTool(String serviceName, Map<String, Object> parameters) {
        try {
            return basicMcpClient.callToolAsText(serviceName, parameters).getContent();
        } catch (Exception e) {
            throw new RuntimeException("工具执行失败: " + serviceName, e);
        }
    }
     ...
     * 记录工具执行日志
     */
    private void logToolExecution(ChatSessionTool session, ToolTask task, Map<String, Object> params) {
        ChatMessage executionMessage = ChatMessage.ofUserTmpl(
                        "执行工具任务 ${taskIndex}: ${serviceName}\n" +
                                "参数: ${parameters}\n" +
                                "意图: ${intentType} (置信度: ${confidence})")
                .paramAdd("taskIndex", task.getTaskIndex())
                .paramAdd("serviceName", task.getServiceName())
                .paramAdd("parameters", JSONUtil.toJsonStr(params))
                .paramAdd("intentType", task.getIntentType().getName())
                .paramAdd("confidence", task.getConfidence())
                .generate();

        session.addMessage(executionMessage);
    }

    /**
     * 解析任务参数（处理变量引用）
     */
    private Map<String, Object> resolveTaskParameters(ToolTask task, ToolExecutionContext context) {
        Map<String, Object> resolvedParams = new HashMap<>();
        Map<String, Object> originalParams = task.getParameters();
        ...
        for (Map.Entry<String, Object> entry : originalParams.entrySet()) {
            ...
            if (value instanceof String) {
                String resolvedValue = resolveParameterValue((String) value, context);
     ...
     * 解析参数值中的变量引用
     */
    private String resolveParameterValue(String value, ToolExecutionContext context) {
        ...
        while (matcher.find()) {
            ...
            String resolvedValue = resolveVariable(variable, context);
     ...
     * 解析单个变量
     */
    private String resolveVariable(String variable, ToolExecutionContext context) {
        // 支持多种变量格式：
        // task_1_result - 特定任务的结果
        // getKeyboardByName_result - 特定工具的结果
        // task_1_field - 特定任务的字段值

        if (variable.startsWith("task_")) {
            String[] parts = variable.split("_", 3);
            if (parts.length >= 3) {
                String taskId = "task_" + parts[1];
                String field = parts.length > 2 ? parts[2] : "result";

                ToolExecutionResult taskResult = context.getTaskResults().get(taskId);
                if (taskResult != null && "result".equals(field)) {
                    return taskResult.getResult();
                }
            }
        } else if (variable.endsWith("_result")) {
            String toolName = variable.substring(0, variable.length() - 7);
            return context.getToolResults().getOrDefault(toolName, "${" + variable + "}");
        }

        return "${" + variable + "}"; // 无法解析，保持原样
    }
     ...
     * @param context
     */
    private void updateTaskStatus(ToolTask task, ToolExecutionContext context) {
        log.info("task status update - {} : {}", task.getTaskId(),task.getStatus());
    }


    /**
     * get executable tasks by dependency order
     * @param tasks
     * @return
     */
    private List<ToolTask> getExecutableTasks(List<ToolTask> tasks) {
        List<ToolTask> executable = new ArrayList<>();
        ...
        while(changed){
            ...
            for (ToolTask task : tasks) {
                if (task.getStatus() == ToolTaskStatus.PENDING &&
                        isTaskReady(task, completedTasks)) {
                    executable.add(task);
        ...
        executable.sort(Comparator.comparingInt(ToolTask::getPriority).reversed());
        return executable;
     ...
     * @return
     */
    private boolean isTaskReady(ToolTask task, Set<String> completedTasks) {
     ...
     * @return
     */
    private ToolExecutionPlan createExecutionPlan(List<IntentAnalysis> actionableIntents,
                                                  String message, String sessionId,
                                                  String username, ChatSessionTool session) {
        ToolExecutionPlan plan = new ToolExecutionPlan();
        plan.setSessionId(sessionId);
        plan.setUsername(username);
        plan.setOriginalMessage(message);
        plan.setCreatedAt(System.currentTimeMillis());

        List<ToolTask> tasks = new ArrayList<>();

        for (IntentAnalysis intentAnalysis : actionableIntents) {
            // extract tool params from response
            Map<String, Object> intentParams = extractMultiToolParamsFromRes(message,
                    intentAnalysis.getIntentType(), sessionId, username, intentAnalysis.getReason(), session);

            List<Map<String, Object>> tools = (List<Map<String, Object>>) intentParams.get("tools");
            if (!CollectionUtils.isEmpty(tools)) {
                for (Map<String, Object> toolConfig : tools) {
                    ToolTask task = createToolTask(toolConfig, intentAnalysis, tasks.size() + 1);
                    tasks.add(task);
                }
            }
        }
    ...
    /**
     * todo persist execution plan to db
     * @param plan
     */
    private void persistExecutionPlan(ToolExecutionPlan plan) {
        String planJson = JSONUtil.toJsonStr(plan);
        log.info("persist execution plan: {}", planJson);
    }

    /**
     * analyze task dependencies and set dependencies field
     * @param tasks
     */
    private void analyzeTaskDependencies(List<ToolTask> tasks) {
        for (int i = 0; i < tasks.size(); i++) {
            ToolTask currentTask = tasks.get(i);
            Map<String, Object> params = currentTask.getParameters();

            if (params != null) {
                //check if params contain reference to other tasks
                List<String> dependencies = findParameterDependencies(params, tasks, i);
                currentTask.setDependencies(dependencies);
            }
        }
    }

    /**
     * find parameter dependencies in tool task parameters
     * @param params
     * @param tasks
     * @param currentIndex
     * @return
     */
    private List<String> findParameterDependencies(Map<String, Object> params, List<ToolTask> tasks, int currentIndex) {
        List<String> dependencies = new ArrayList<>();

        for (Object value : params.values()) {
            if (value instanceof String) {
                String stringValue = (String) value;
                // 查找对之前任务的引用，格式如 ${task_1_result} 或 ${getKeyboardByName_result}
                Pattern pattern = Pattern.compile("\\$\\{(task_(\\d+)_result|([a-zA-Z_]+)_result)\\}");
                Matcher matcher = pattern.matcher(stringValue);

                while (matcher.find()) {
                    String refType = matcher.group(1);
                    if (refType.startsWith("task_")) {
                        String taskIndexStr = matcher.group(2);
                        int taskIndex = Integer.parseInt(taskIndexStr);
                        if (taskIndex < currentIndex) {
                            dependencies.add("task_" + taskIndex);
                        }
                    } else {
                        String toolName = matcher.group(3);
                        // 查找之前执行过的同名工具
                        for (int i = 0; i < currentIndex; i++) {
                            if (tasks.get(i).getServiceName().equals(toolName)) {
                                dependencies.add(tasks.get(i).getTaskId());
                                break;
                            }
                        }
                    }
    ...
    /**
     * create tool task from tool config
     * @param toolConfig
     * @param intentAnalysis
     * @param taskIndex
     * @return
     */
    private ToolTask createToolTask(Map<String, Object> toolConfig, IntentAnalysis intentAnalysis, int taskIndex) {
        ToolTask task = new ToolTask();
        task.setTaskId(UUID.randomUUID().toString());
        task.setTaskIndex(taskIndex);
        task.setServiceName((String) toolConfig.get("serviceName"));
        task.setParameters((Map<String, Object>) toolConfig.get("parameters"));
        task.setIntentType(intentAnalysis.getIntentType());
        task.setConfidence(intentAnalysis.getConfidence());
        task.setReason(intentAnalysis.getReason());
        task.setPriority((Integer) toolConfig.getOrDefault("priority", 5));
        task.setStatus(ToolTaskStatus.PENDING);
